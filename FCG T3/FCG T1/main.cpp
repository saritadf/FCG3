...
//const std::string volume_file = "stent16.raw";
//const std::string volume_file = "colon_phantom16.raw";
//
...
//------------------ANTweakBAR----------
TwBar *myBar;
const std::string volume_file = "mri_ventricles.raw";
	//const std::string volume_file = "head256.raw";

void TW_CALL mri_ventricles(void *){
		const std::string volume_file = "mri_ventricles.raw";
...
void TW_CALL skull(void *){
		const std::string	volume_file = "skull.raw";
		selected=TRUE;
		const int XDIM = 256;
		const int YDIM = 256;
		const int ZDIM = 256;
}
void TW_CALL aneurism(void *){
		const std::string	 volume_file = "aneurism.raw";
		selected=TRUE;
		const int XDIM = 256;
		const int YDIM = 256;
		const int ZDIM = 256;
}
void TW_CALL bonsai(void *){
	const std::string volume_file = "bonsai.raw";
	selected=TRUE;
	const int XDIM = 256;
	const int YDIM = 256;
	const int ZDIM = 256;
}
void TW_CALL engine(void *){
...
bool LoadVolume() {
	...
	if(infile.good()) {
		...
		long long ex;
		ex=sizeof(GLushort);
		ex=XDIM*YDIM*ZDIM;//*sizeof(GLushort));
...
//	if(TwEventMouseButtonGLUT(button,s,x,y)) return;
	if (s == GLUT_DOWN)
	{
		oldX = x;
		oldY = y;
...
void OnResize(int w, int h) {
	...
	Pe = glm::perspective(60.0f,(float)w/h, 0.1f,1000.0f);
}
...
int main(int argc, char** argv) {
	...
	/*
	TwAddButton(myBar,"botones", NULL, NULL, " label='VOLUMENES' ");//comentario 
	 TwAddButton(myBar, "Skull", skull, NULL, " label='Skull' ");
 TwAddButton(myBar, "Aneurism", aneurism, NULL, " label='Aneurism' ");
 TwAddButton(myBar, "Bonsai", bonsai, NULL, " label='Bonsai' ");
 TwAddButton(myBar, "Engine", engine, NULL, " label='Engine' ");
 TwAddButton(myBar, "Head", head, NULL, " label='Head' ");*/
